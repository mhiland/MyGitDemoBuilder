...
namespace GitDemo.UnitTests.IntegrationTests
{
    public class GitObjectTests : TestFixtureBase
    {
        ...
        {
            var file = Path.Combine(GitDirectory.FullName, ".git", "objects", "E6", "9DE29BB2D1D6434B8B29AE775AD8C2E48C5391");
        ...
        {
            var file = Path.Combine(GitDirectory.FullName, ".git", "objects", "4D", "E1EF196746A289866677488A7081869EA1A593");
            Compression.DecompressData(File.ReadAllBytes(file), out var outData);
            var actual = Encoding.UTF8.GetString(outData);

            const string expected = "tree 63\0100644 exampleFile.txt\0e69de29bb2d1d6434b8b29ae775ad8c2e48c5391";
        ...
        {
            var file = Path.Combine(GitDirectory.FullName, ".git", "objects", "3C", "D20A70F3BEC41ECBC47EB8434F0B9197601310");
        ...
        {
            var file = Path.Combine(GitDirectory.FullName, ".git", "logs", "HEAD");
        ...
        {
            var file = Path.Combine(GitDirectory.FullName, ".git", "logs", "refs", "heads", "main");
        ...
        {
            var file = Path.Combine(GitDirectory.FullName, ".git", "refs", "heads", "main");
        ...
        {
            var file = Path.Combine(GitDirectory.FullName, ".git", "objects", "0D", "B50E0D48F0A7A121CE592487E0603E5268713C");
